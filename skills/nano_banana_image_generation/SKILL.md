...
The `generate_image` tool defaults to saving images in the conversation's artifact directory. You MUST immediately move or copy the generated image file into the project's `/images/[category]/` directory. Additionally, you must save the JSON prompt used to generate the image into the corresponding `/prompts/[category]/` directory (e.g., `/prompts/miscellaneous/`). Refer to `gemini.md` for full project structure rules.
