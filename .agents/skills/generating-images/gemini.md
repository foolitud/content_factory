...
1. Use the `generate_image` tool with the Nano Banana json prompt structure to generate the image based on the user's requirements. **Note:** The tool saves the image to a temporary artifact directory by default.
2. **Move or copy the resulting image** from the artifact directory to the correctly categorized subfolder inside the project's `/images/` directory (e.g., `/images/infographics/`). **If a specific category cannot be determined, save it to `/images/miscellaneous/`.** This is mandatory for every single generation.
3. Save the corresponding JSON prompt configuration as a file to the `/prompts/` directory, mirroring the exact same subfolder used for the image (e.g., `/prompts/infographics/` or `/prompts/miscellaneous/`).
